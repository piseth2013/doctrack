/*
  # Fix logo upload policies

  This migration fixes the RLS policies for logo uploads by:
  1. Dropping existing conflicting policies
  2. Creating new policies with correct permissions
  3. Ensuring proper bucket configuration
*/
...
CREATE POLICY "organization_assets_public_select" ON storage.objects
  FOR SELECT TO public
  USING (bucket_id = 'organization_assets');

CREATE POLICY "organization_assets_admin_insert" ON storage.objects
...
CREATE POLICY "organization_assets_admin_update" ON storage.objects
...
CREATE POLICY "organization_assets_admin_delete" ON storage.objects
